(function ($) {
  ...
  $(document).ready(function () {
    ...
    function toggleSubMenu() {
      if ($(window).width() <= 991) {
        // Remove previous handlers that toggled the whole item so links can navigate
        $(".has-submenu").off("click");

        // Toggle only when the caret/button is clicked - keep the link as a real navigation anchor
        $(".nav-submenu-toggle").off("click").on("click", function (e) {
          e.preventDefault();
          e.stopPropagation();
          var $btn = $(this);
          var $item = $btn.closest('.has-submenu');
          var $submenu = $item.children('.nav-submenu').first();

          // Close sibling submenus at the same level
          $item.siblings('.has-submenu').each(function () {
            $(this).removeClass('active');
            $(this).children('.nav-submenu').stop(true, true).slideUp(300).attr('aria-hidden', 'true');
            $(this).find('.nav-submenu-toggle').attr('aria-expanded', 'false');
          });
          ...
          if ($submenu.length) {
            $submenu.stop(true, true).slideToggle(300).attr('aria-hidden', !willOpen);
          }
        });
      } else {
        $(".nav-submenu-toggle").off("click");
        $(".has-submenu").off("click");
      }
    }
    ...
    // Desktop hover fallback to reliably open nested submenus to the right
    function navHoverFallback() {
      if ($(window).width() > 991) {
        // top-level Services dropdown open by JS for better reliability in some browsers
        $('.header .nav-menu > .nav-menu__item.has-submenu').off('.navHover').on('mouseenter.navHover', function () {
          $(this).children('.nav-submenu').addClass('js-open');
        }).on('mouseleave.navHover', function () {
          $(this).children('.nav-submenu').removeClass('js-open');
        });

        // nested submenu items show right-side flyout
        $('.nav-submenu__item.has-submenu, .nav-menu__item.has-submenu').off('.navHoverSub').on('mouseenter.navHoverSub', function () {
          $(this).children('.nav-submenu').addClass('js-open');
        }).on('mouseleave.navHoverSub', function () {
          $(this).children('.nav-submenu').removeClass('js-open');
        });
      } else {
        $('.header .nav-menu > .nav-menu__item.has-submenu').off('.navHover');
        $('.nav-submenu__item.has-submenu, .nav-menu__item.has-submenu').off('.navHoverSub');
      }
    }

    navHoverFallback();
    $(window).on('resize', navHoverFallback);
